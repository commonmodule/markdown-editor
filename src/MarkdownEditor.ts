...
export default class MarkdownEditor extends DomNode {
  ...
  private setCaretOffset(element: HTMLElement, offset: number) {
    if (offset < 0) return;
  ...
  private parseMarkdownWithSpans(text: string): string {
    let i = 0;
    let result = "";

    const stack: Array<"strongem" | "strong" | "em"> = [];
